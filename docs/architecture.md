...
7. Runs the selected harness, OpenCode by default or Claude Code when
   configured.
...
- Notifier adapter: posts job updates to chat, issue trackers, or other
  systems.

Adapter interfaces live in `packages/core`. The worker may ship built-in
adapters, but provider-specific behavior must remain optional and selected by
profile, metadata, or deployment config.
...
A job with `kind: "verify"` checks human-readable checklist items against its
requested `ref` (or `origin/main` when no ref is supplied). The harness writes
`check-results.json` in the job artifact directory as either a bare result
...
job metadata, checklist results, artifact metadata, and the pull request when
one exists. Deployments can sign the exact JSON body with
`URIEL_CALLBACK_SECRET` using HMAC-SHA256.
